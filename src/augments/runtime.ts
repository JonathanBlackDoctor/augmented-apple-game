...
function eligible(tier: AugTier, owned: string[]): Augment[] {
...
/** Offer 3 distinct augment ids of `tier`, excluding owned/conflicts. */
export function rollOffer(tier: AugTier, rng: SeededRng, owned: string[]): string[] {
  const pool = eligible(tier, owned).map((a) => a.id);
